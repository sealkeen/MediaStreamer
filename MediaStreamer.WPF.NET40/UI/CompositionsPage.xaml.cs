...
namespace DMultHandler
{
    ...
    public partial class CompositionsPage : FirstFMPage
    {
        public CompositionsPage()
        {
            ...
            //Session.MainWindow.mainFrame.Content = Session.loadingPage; //LoadManagementElements(); //ListCompositionsAsync();
            Compositions = new List<IComposition>();
            Queue = new LinkedList<Composition>();
            InitializeComponent();
            DataContext = this;
            //tsk.Wait(); //Session.MainWindow.mainFrame.Content = this;
        }
        ...
        public virtual List<IComposition> GetICompositions()
        {
            try {
                ...
                Dispatcher.BeginInvoke(new Action(() => Session.MainWindow.SetContentPageCompositions()));
        ...
        public void PlayTarget(IComposition target)
        {
            try
            {
                ...
                Session.MainWindow.SetCurrentAction($"Now playing {art ?? "Unknown"} – " + $"{comp ?? "Unknown"}");
        ...
        public void ChangeComposition(IList selectedItems)
        {
            try
            {
                if (selectedItems.Count >= 0)
                {
                    ...
                    Session.MainWindow.mainFrame.Content = Session.TagEditorPage;
                }
            }
            catch (Exception ex)
            {
